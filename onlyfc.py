...
def inference_c3d(_X, _dropout, batch_size, _weights, _biases):
  ...
  dense1 = tf.reshape(_X, [batch_size, _weights['out1'].get_shape().as_list()[0]]) # Reshape conv3 output to fit dense layer input
  dense1 = tf.matmul(dense1, _weights['out1']) + _biases['out1']

  dense1 = tf.nn.relu(dense1, name='fc1') # Relu activation
  dense1 = tf.nn.dropout(dense1, _dropout)

  # Output: class prediction
  out = tf.matmul(dense1, _weights['out2']) + _biases['out2']
